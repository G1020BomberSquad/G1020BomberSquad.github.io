...
if (isset($_POST['reg_user'])) {
  ...
  if ($user) { // if user exists
    ...
    if ($user['email'] === $email) {
      array_push($errors, "Met dit email is al een account geregistreerd");
    }
  }
...
if (isset($_POST['login_user'])) {
   ...
   if (empty($username)) {
    ...
    if (empty($password)) {
        array_push($errors, "Wachtwoord is veplicht");
    }
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($results) == 1) {
          ...
          header('location: index.php');
            ...
            array_push($errors, "Uw gebruikersnaam/wachtwoord is verkeerd");
        }
    }
  }
